...
<div style="width:60%; overflow:hidden;" id="panelRight">
	<div id="boletines_url"></div>
</div>
<?php
	$actions = $functions->actions($_SESSION['session']['permissions'],'Boletines');
	...
	if($actions!=''){	
		foreach($actions as $key => $value){
			...
			if ($template!='editarboletin') {
		     ...
		     if ($template == 'editarboletin' || $template == 'nuevoboletin'){
				$sw = 1;
			}
		};
	}	
...
$(document).ready(function(){
	...
	var sw = <?php print $sw; ?>;
	var d = <?php print json_encode($informacion->boletines($sw)); ?>;
	...
	$('#grid').grid({
		...
		onloadgrid:function(){
			var idBoletin = document.getElementById('idid0');
			var indexRecord = 0;
			
			$.each(d,function(i,rcd){
				if(rcd.id == idBoletin.innerHTML){
					indexRecord = i;
				}
			});
		...
		onclickrcd: function(rcd){
			...
			var idBoletin = document.getElementById('idid'+idrcd);
			var indexRecord = 0;
	...
	function viewdetails(data){
		...
		var nArchivo = document.getElementById('boletines_url');

		$('#boletines_url embed').remove();
